...
class roles {
	...
	function editRoles($item) {
		...
		while ($fetch = $run->fetch_assoc()) {
			...
			$internal = $optionBuilder->showUsers(99, $user);
	...
	function saveRoles() {
		...
		if (!empty($item)) {
			...
			$sql = "INSERT INTO `roles` (`ID`, `RoleName`,  `RoleLevel`, `RoleGroup`) VALUES (NULL, '$name', '$level', '$group');";
		}
	...
	public function permittedRoles($item) {
			...
			foreach($functions as $setting){
				...
				$sql = "INSERT IGNORE INTO `functions-permissions` (`ID`, `FunctionID`, `RoleID`, `Admin`, `Added`) VALUES (NULL, '". $setting["selected"] ."', '$item', '$uid', NOW());";
	...
	function manageRoles() {
		...
		$sql = "SELECT `roles`.ID, `roles`.RoleName, `roles`.RoleLevel, count(`access`.ID) 
			FROM `roles`
			LEFT JOIN `access` ON `access`.RoleID = `roles`.ID GROUP BY `roles`.ID";
