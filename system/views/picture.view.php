...
class picture{
	...
	public function deletePicture($item){
		$path = '/var/www/html/datastore/identities/pictures/'.$item;
		unlink($path);
		echo'<div class="successpopup">PHOTO DELETED</div>';
		$this->makePicture($item);
	}
	...
	public function savePicture($item){
		...
		$url = '/datastore/identities/pictures/'. $item .'.png';

		file_put_contents($url, $data);
	}
	
	public function gridPicture($item){
		$path = '/var/www/html/datastore/identities/pictures/';
		$files = scandir($path);

		foreach($files as $file) {
			$file = trim($file);
			$taken = date('Y-m', filemtime($path.$file));

			$uid = str_replace('.png', '', $file);
			if($file == '.' || $file == '..' || $uid == '' ||  $file == ''){
				continue;
			}
			if(is_numeric($item) && strlen($item) == 4 ) {
				if (substr($file,0,5) == $item.'0') {
					echo '<a href=/picture/delete/'.$file.'/'.$item.'><img src="/datastore/identities/pictures/'. $file .'" width="100px"></a>';
					$i++;
				}
				
			} else {
				echo '<a href=/picture/delete/'.$file.'><img src="/datastore/identities/pictures/'. $file .'" width="100px"></a>';
				$i++;
			}
			
		}	
		echo "Total pictures taken $i";
	}
