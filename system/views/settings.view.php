...
class settings {
	...
	public function saveSettings($item) {
		if($item == "permissions"){
			...
			foreach(array_keys($_POST['titles']) as $id){
				...
				$functions["$id"]["icon"] = $_POST['icon'][$id];
			}
			...
			foreach($functions as $setting){
				...
				if($role != ''){
					$role = "`FunctionRequiredPermissions` =  '". $setting["role"] ."',";
				}
			...
			echo '<div class="successpopup">Settings saved</div>';
	...
	public function permissionsSettings() {
		...
		$dir = 'templates/mobile/bootstrap-icons';
		if ($handle = opendir($dir)) {
		...
		while ($fetch = $run->fetch_row()) {
			...
			if($current != $fetch[1]){
				...
				<div class="label" style="width:40px;"><i>Icon</i></div></div>';
			...
			if ($exists){
				echo'  <div style="clear:both">
							<div class="label"  style="width:70px;"><b>' . ucwords($fetch[2]) . '</b></div>
							...
							<div class="label" style="width:50px;"><input type="text" class="submit" name="menu[' . $fetch[0] . ']" style="width: 50px;" value="' . $menu . '">  </div>
							<div class="label" style="width:150px;"><select  class="submit" name="icon[' . $fetch[0] . ']" style="width: 150px;"><option>-</option> '.$list.' </select> </div>
						</div>';
			} else{
				$sql = "DELETE FROM `functions` WHERE `Class` = '".$fetch[1]."' AND `Function` = '$function'";
		...
		$sql = "DELETE FROM `functions` WHERE `Class` NOT IN ('$list')";
		$this->core->database->doSelectQuery($sql);
