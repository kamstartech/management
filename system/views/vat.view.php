...
class vat{
	...
	public function manageVat() {
		...
		$sql = "SELECT *, `transactions`.ID as PID, MONTH(`TransactionDate`) as month
		FROM `transactions`
		...
		while ($fetch = $run->fetch_assoc()) {
			...
			if($month != $prevmonth && $i > 0){
				...
				$monthwritten = $dateObj->format('F'); 
				
				$output .= '<tr style="background-color: #CCC;">
				<td><b>Total '.$monthwritten.'</b></td>
				<td></td>
				<td></td>
				<td><b>' . number_format($totalmonth) . ' ZMW</b></td>
				<td><b>' . number_format($vatmonth) . ' ZMW</b></td>
			...
			if($Type == 1){
				$vat = $amount-($amount/1.16);
			} else {
				$vat = 0;
			}
			
			$vattotal =  $vat+$vattotal;
			$vatmonth =  $vat+$vatmonth;
		...
		$output .= '<tr style="background-color: #999;">
			<td><b>TOTAL REVENUE</b></td>
			<td><b></b></td>
			<td></td>
			<td><b>' . number_format($total) . ' ZMW</b></td>
			<td><b>' . number_format($vattotal) . ' ZMW</b></td>
			<td><a href="' . $this->core->conf['conf']['path'] . '/clients/show/'. $userid .'?payid='.$userid.'&date='.$today.'">'.$name.'</a></td>
			<td></td>
			<td></td>';
