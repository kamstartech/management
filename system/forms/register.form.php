<div id="studentheader" class="studentname"><?php $count=1; echo $count; ?> - General information</div>
...
$( ".sponsorcontacts" ).slideUp( "slow" );
$( ".programdetails" ).slideUp( "slow" );
$( ".submitdetails" ).slideUp( "slow" );
$( ".supportingdetails" ).slideUp( "slow" );
});
</script>
...
<div class="studentdetails formElement" style="display: none; margin-bottom: 20px;">
	...
	<tr>
		...
		<td>
			<?php
				if($existing=="yes"){
					echo'<input type="text" name="studentid" id="username" required>'; 
				}else{
					echo'<input type="text" name="studentid" id="username" required>';
				}
			?>
	...
	<tr>
		...
		<td>
			<input type="search" name="middlename" /></td>
		<td>Optional</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>Minimum age for applicants is 16 years</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<input type="text" name="streetname" id="streetname" required></td>
	...
	<tr>
		...
		<td>
			<input type="text" name="celphone" id="phone" required>
		</td>
...
if($this->core->cleanGet['existing'] != "yes"){
	...
	<tbody>';
		...
		while ($fetch = $run->fetch_row()) {
			...
			echo'<td>'.$fetch[5].'</td><td><input name="grade['.$fetch[2].']" type="search" class="grade"></td>';
			$i++;
		}
		...
		if($i ==0){
			...
			while ($fetch = $run->fetch_row()) {
				...
				echo'<td>'.$fetch[1].'</td><td><input name="grade['.$fetch[0].']" type="search" class="grade"></td>';
				$i++;
			}
		}
...
<p>In this section you can provide contact information that should be used in case of an emergency, in general this should be a family member or legal guardian. Use the add button to add as many emergency contacts as you wish.</p>
	...
	<div class="emergencycontact">
		...
		<table width="768" height="135" border="0" cellpadding="5" cellspacing="0">
			...
			<tr>
				...
				<td>
					<select name="econtact[0][relationship]" id="econtact_0_relationship" data-pattern-name="econtact[++][relationship]" data-pattern-id="contact_++_relationship">
						...
						<option value="Grandparrent">Grandparent</option>
			...
			<tr>
				<td height="22"><strong><?php $countsub++; echo $count .'.'. $countsub . ' - '; ?> Email</strong></td>
				<td>
...
<div id="sponsorheader" class="studentname"><?php $count++; echo $count; ?> - Sponsors information</div>
...
<p>In this section you must provide contact information for your sponsor, if you are self sponsored please indicate this.</p>

	<div class="sponsorcontact">
		<input type="hidden" name="scontact[0][id]" data-pattern-name="scontact[++][id]" data-pattern-id="scontact_++_index"/>
		...
		<table width="768" height="135" border="0" cellpadding="5" cellspacing="0">
			<tr>
				...
				<td width="200" bgcolor="#EEEEEE"><strong>Input fields</strong></td>
				<td bgcolor="#EEEEEE">
					<div class="deletesponsorcontact">
						<a href="#"><b>REMOVE RECORD</b></a>
					</div>
				</td>
			</tr>
			<tr>
				<td height="19"><strong><?php $countsub++; echo $count .'.'. $countsub . ' - '; ?> Full Name</strong></td>
				<td>
					<input type="text" name="scontact[0][fullname]" id="scontact_0_fullname" data-pattern-name="scontact[++][fullname]" data-pattern-id="scontact_++_fullname"/></td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<td height="19"><strong><?php $countsub++; echo $count .'.'. $countsub . ' - '; ?> Sponsorship</strong></td>
				<td>
					<select name="scontact[0][relationship]" id="scontact_0_relationship" data-pattern-name="scontact[++][relationship]" data-pattern-id="scontact_++_relationship">
						<option value="Sponsor" selected="selected">Self Sponsored</option>
						<option value="Spouse">Spouse</option>
						<option value="Parent">Parent</option>
						<option value="Guardian">Legal Guardian</option>
						<option value="Religous Organization">Religious Organization</option>
			...
			<tr>
				<td height="22"><strong><?php $countsub++; echo $count .'.'. $countsub . ' - '; ?> Phone Number</strong></td>
				<td>
					<input type="text" name="scontact[0][phonenumber]" id="scontact_0_phonenumber" data-pattern-name="scontact[++][phonenumber]" data-pattern-id="scontact_++_phonenumber"/></td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<td height="22"><strong><?php $countsub++; echo $count .'.'. $countsub . ' - '; ?> Email</strong></td>
				<td>
					<input type="text" name="scontact[0][street]" id="scontact_0_street" data-pattern-name="scontact[++][street]" data-pattern-id="scontact_++_street"/></td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<td height="22"><strong><?php $countsub++; echo $count .'.'. $countsub . ' - '; ?> Address</strong></td>
				<td><input type="text" name="scontact[0][town]" id="scontact_0_town" data-pattern-name="scontact[++][town]" data-pattern-id="scontact_++_town"/></td>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<td height="22"><strong><?php $countsub++; echo $count .'.'. $countsub . ' - '; ?> Town</strong></td>
				<td><input type="text" name="scontact[0][postalcode]" id="scontact_0_postalcode" data-pattern-name="scontact[++][postalcode]" data-pattern-id="scontact_++_postalcode"/></td>
				<td>&nbsp;</td>
			</tr>
		</table>

		<div class="addsponsorcontact" style="margin-left: 10px; padding: 20px;">
...
<script type="text/javascript">
 $('.sponsorcontacts').repeater({
	btnAddClass: 'addsponsorcontact',
	btnRemoveClass: 'deletesponsorcontact',
	groupClass: 'sponsorcontact',
	minItems: 1,
	maxItems: 0,
	startingIndex: 0,
	reindexOnDelete: true,
...
<?php if($stype != "d" && $stype != "c"){ ?>
<div id="programheader" class="studentname"><?php $count++; echo $count; ?> - Second and third program choices</div>
...
<div class="programdetails formElement" style="display: none;  margin-bottom: 20px;">
	<p>This is the final part of your admission request, here you will need to select your second and third choice program choices, should you not get picked for your main program.</p>
...
<?php if($stype == "m"){ ?>
<div id="supportingheader" class="studentname"><?php $count++; echo $count; ?> - Supporting Statement</div>

<script>
$( "#supportingheader" ).click(function() {
$( ".educationalrecords" ).slideUp( "slow" );
$( ".studentdetails" ).slideUp( "slow" );
$( ".results" ).slideUp( "slow" );
$( ".emergencycontacts" ).slideUp( "slow" );
...
$( ".programdetails" ).slideUp( "slow" );
$( ".submitdetails" ).slideUp( "slow" );
$( ".supportingdetails" ).slideToggle( "slow" );
});
</script>

<?php $countsub = 0; ?>

<div class="supportingdetails formElement" style="display: none;  margin-bottom: 20px;">
	<p>Make a brief statement explaining/motivating why you have chosen to apply the programme.</p>
	...
	<table width="768" height="94" border="0" cellpadding="5" cellspacing="0">
		...
		<tr>
				<td height="22"><strong> Document with statement.</strong></td>
				<td>
					<input type="file" name="statement" id="statement" /></td>
				<td>Provide a written motivation.</td>
			</tr>
		</table>
...
$( "#submitheader" ).click(function() {
	...
	field = input.name;
		...
		not_pass = true;
		$("input[name='" + field + "']").css('border', '3px solid red');
           	break;
        } else {
		$("input[name='" + field + "']").css('border', '2px solid green');
		not_pass = false;
        }
    ...
    if (not_pass) {
        $("#errorpopup").show();
        $("#errorpopup").html("You are still missing some fields. Check the field(s) marked red");
	$("#registersubmit").hide();
	console.log("Validation failed " + inputs.length);
        return false;
    } else {
	$("#errorpopup").hide();
	$("#registersubmit").show();

	console.log("Validation passed " + inputs.length);
    }
});
...
<div class="errorpopup" id="errorpopup"></div>
	...
	<?php
	if($this->core->cleanGet['existing'] == "yes"){
		echo'<input type="submit" class="register" name="submit-registrar" id="registersubmit" value="UPDATE INFORMATION"/>';
	}else{
		echo'<input type="submit" class="register" name="submit-registrar" id="registersubmit" value="SUBMIT APPLICATION"/>';
	}
	?>
</div>
